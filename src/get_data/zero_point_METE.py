...
def find_roots(initial_lambdas, state_variables, values={'dN/S': 0, 'dS': 0}, method="METimE", data_set="dummy", scaling=False):
    ...
    # def track_progress(x, progress_bar):
    #     progress_bar.update(1)
    #
    #
    # def fsolve_with_progress(func, lambdas, max_iterations):
    #     current_guess = lambdas
    #     for i in range(max_iterations):
    #         current_guess = fsolve(func, current_guess, maxfev=1,
    #                                xtol=1e-14)
    #         track_progress(current_guess, progress_bar)
    #     return current_guess
    #
    # solution = fsolve_with_progress(func, lambdas, max_iterations)

    start_time = time.time()
    solution = fsolve(func, lambdas, args=args, maxfev=max_iterations, xtol=1e-14, epsfcn=1e-14)
    ...
    # if method == "METimE":
    #     if data_set == "fish":
    #         initial_guess = initial_lambdas + [0, 0]
    #         solution = fsolve(METimE_fish_functions,
    #                         initial_guess,
    ...
    #     elif data_set == "birds":
    #         initial_guess = initial_lambdas + [0, 0]
    #         solution = fsolve(METimE_birds_functions,
    #                           initial_guess,
    #                           args=(state_variables, values, scaling),
    #                           maxfev=20_000,
    #                           xtol=1e-14)
    #     else:
    #         initial_guess = initial_lambdas + [0, 0, 0]
    #         solution = fsolve(METimE_BCI_functions,
    #                           initial_guess,
    #                           args=(state_variables, values, scaling),
    #                           maxfev=20_000,
    #                           xtol=1e-14)
    # elif method == "METE":
    #     solution = fsolve(METE_functions,
    #                     initial_lambdas,
    ...
    #     initial_guess = initial_lambdas + [0, 0, 0]
    #     solution = fsolve(dynaMETE_functions,
    #                       initial_guess,
    #                       args=(state_variables, values, data_set, scaling),
    #                       maxfev=20_000,
    #                       xtol=1e-14)
    # else:
    #     solution = fsolve(meteR_functions,
    #                       initial_lambdas,
    #                       args=(state_variables, scaling),
    #                       maxfev=20_000,
    #                       xtol=1e-14)
...
def METimE_birds_functions(lambdas, X, values, scaling):
    ...
    # Calculate all exponentials separately
    # a1_l3_n2 = [exp(x) for x in -a1*l3*n**2]
    # c1_l4_n2 = [exp(x) for x in -c1*l4*n**2]
    # l1_n = [exp(x) for x in -l1*n]
    # E2_a4_l3_n = [exp(x) for x in -E**2*a4*l3*n]
    # E2_c4_l4_n = [exp(x) for x in -E**2*c4*l4*n]
    # E_l2_n = [exp(x) for x in -E*l2*n]
    # a3_l3 = exp(a3*l3)
    # c3_l4 = exp(c3*l4)
    # a3_plus = [x for x in a3 + E**2 * a4 * n]
    # c3_plus = [x for x in c3 + E**2 * c4 * n]
    ...
    # LAMBDA = [
    #     a * b * c * (a3_l3 * c3_l4 - f * g * h)
    #     for a, b, c, f, g, h in zip(a1_l3_n2, c1_l4_n2, l1_n, E2_a4_l3_n, E2_c4_l4_n, E_l2_n)
    # ]
    ...
    # PSI = [
    #     a * b * c * a3_l3 * c3_l4 * f
    #     for a, b, c, f in zip(a1_l3_n2, c1_l4_n2, l1_n, a3_plus)
    # ]

    # OMEGA = [
    #     a * b * c * a3_l3 * c3_l4 * f
    #     for a, b, c, f in zip(a1_l3_n2, c1_l4_n2, l1_n, c3_plus)
    # ]
...
def plot_rank_abundance(N, S, sad_METimE, sad_METE, sad_meteR, sad_dynaMETE, empirical_rank_sad, census):
    ...
    # # # For BCI, only look at first 20 species
    # if data_set == "BCI":
    #     empirical_rank_sad = empirical_rank_sad[:30]
    #
    #     idx = np.searchsorted(sorted_expected_METE, 30, side="right")
    #     sorted_expected_METE = sorted_expected_METE[:idx]
    #     x_positions_METE= x_positions_METE[:idx]
    #
    #     idx = np.searchsorted(sorted_expected_METimE, 30, side="right")
    #     sorted_expected_METimE = sorted_expected_METimE[:idx]
    #     x_positions_METimE= x_positions_METimE[:idx]
    #
    #     idx = np.searchsorted(sorted_expected_meteR, 30, side="right")
    #     sorted_expected_meteR = sorted_expected_meteR[:idx]
    #     x_positions_meteR= x_positions_meteR[:idx]
    #
    #     idx = np.searchsorted(sorted_expected_dynaMETE, 30, side="right")
    #     sorted_expected_dynaMETE = sorted_expected_dynaMETE[:idx]
    #     x_positions_dynaMETE= x_positions_dynaMETE[:idx]
    ...
    axes[0].set_ylabel('Population Size (n)', fontsize=14)
    axes[0].set_title('METE Rank Abundance', fontsize=20)
    ...
    axes[1].set_ylabel('Population Size (n)', fontsize=14)
    axes[1].set_title('meteR Rank Abundance', fontsize=20)
    ...
    axes[2].set_ylabel('Population Size (n)', fontsize=14)
    axes[2].set_title('METimE Rank Abundance', fontsize=20)
    ...
    if data_set != "fish":
        ...
        axes[3].set_ylabel('Population Size (n)', fontsize=14)
        axes[3].set_title('dynaMETE Rank Abundance', fontsize=20)
    ...
    file_name = f"../../results/{scaling}_{data_set}_{census}.pdf"
    #plt.savefig(file_name, format='pdf', dpi=150)
...
if __name__ == "__main__":
    ...
    for scaling in ['True', 'False']:
        for data_set in ['fish', 'birds', 'BCI']:
            ...
            for (idx1, values), (idx2, state_var), (rank_sad) in zip(values_df.iterrows(), state_var_df.iterrows(), rank_df):
                ...
                for method in methods:
                    ...
                    quantile_errors = calculate_goodness_of_fit(quantile_function(theoretical_sad, state_var['S'], state_var['N']), rank_sad, n_param)
