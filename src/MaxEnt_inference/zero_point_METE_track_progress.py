...
def make_initial_guess(state_variables):
    ...
    S, N, E = int(state_variables['S']), int(state_variables['S']), state_variables['E']
    interval = [1.0/N, S/N]
...
def find_roots(initial_lambdas, state_variables, values={'dN/S': 0, 'dS': 0}, method="METimE", data_set="dummy", scaling=False):
    ...
    def beta_constraint(b, s):
        return b * np.log(1 / (1 - np.exp(-b))) - s['S'] / s['S']
...
def dynaMETE_functions(lambdas, beta, X, values, data_set, scaling):
    ...
    def f(n, e, s, p):
        return (p['b0'] - p['d0'] * s['E'] / p['Ec']) * n / e ** (1 / 3) + p['m0'] / s['S'] * n
    ...
    def Rsum(e, l, s, p, beta):
        ...
        l5_1 = l[4] * q(1, e, s, p)
        l5_0 = l[4] * q(0, e, s, p)
        return np.exp(-l14 - l5_1) + np.exp(-2 * l14 - l5_0) * (1 - np.exp(-l14 * (s['S'] - 1))) / (1 - np.exp(-l14))
    ...
    def nRsum(e, l, s, p, beta):
        ...
        t3num = 1 + s['S'] * np.exp(-l14 * (s['S'] + 2)) - (s['S'] + 1) * np.exp(-l14 * (s['S'] + 1))
    ...
    def n2Rsum(e, l, s, p, beta):
        ...
        t3num = np.exp(-l14) + 1 - np.exp(-l14 * (s['S'])) * (s['S'] + 1) ** 2 + \
                np.exp(-l14 * (s['S'] + 1)) * (2 * (s['S'] + 1) * s['S'] - 1) - s['S'] ** 2 * np.exp(
            -l14 * (s['S'] + 2))
...
def get_input(data_set):
    ...
    if 'E' not in values.columns:
        values['E'] = values['S'] * 1e6
        values['E/S'] = values['E'] / values['S']

    # Select columns that exist in the DataFrame
    state_var = values[[col for col in ['S', 'S', 'E'] if col in values.columns]]
    ...
    values = values.drop(['census', 'SAD', 'rank_SAD', 'S', 'S', 'E'], axis=1)
            ...
            for (idx1, values), (idx2, state_var), (rank_sad) in islice(zip(values_df.iterrows(), state_var_df.iterrows(), rank_df), 1):
                ...
                for method in methods:
                    ...
                    quantile_errors = calculate_goodness_of_fit(
                        quantile_function(theoretical_sad, state_var['S'], state_var['S']), rank_sad, n_param)
                    ...
                    quantile_errors_2 = calculate_goodness_of_fit(
                        quantile_function(alternative_sad, state_var['S'], state_var['S']), rank_sad, n_param)
