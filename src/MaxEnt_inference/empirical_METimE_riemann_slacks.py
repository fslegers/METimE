...
def penalized_entropy_grad(vars, func_vals, macro_var, scales, slack_weight = 1.0):
    ...
    grad[4] = 2 * slack_weight * slacks[0]
    grad[5] = 2 * slack_weight * slacks[1]
...
def inequality_constraint_ub(vars, func_vals, func_index, target_value, scales):
    """
    One side of inequality:

    expected <= (target + s * target) if target >= 0
    expected <= (target - s * target) if target < 0
    ...
    # Expected value
    expected_value = np.sum(R * func_vals[func_index])
    slack = slacks[func_index - 2]

    if target_value < 0:
        return expected_value - (1 + slack) * target_value

    else:
        return expected_value - (1 - slack) * target_value

def inequality_constraint_lb(vars, func_vals, func_index, target_value, scales):
    """
    One side of inequality:
    ...
    if target_value < 0:
        return (1 - slack) * target_value - expected_value
...
def plot_RADs(empirical_rad, METE_rad, METimE_rad, save_name, obs_label='Empirical', use_log=False):
