...
Assumption:
    ...
    Z = sum from n = 1 to S of int e^(- lambda_1 n - lambda_2 n e) from e = 0 to E
...
def find_roots(initial_lambdas, state_variables, values={'dN/S': 0, 'dS': 0}, method="METimE", data_set="dummy", scaling=False):
    ...
    :param state_variables: Dictionary of state variables (e.g., S, S, E).
...
def dynaMETE_functions(lambdas, beta, X, values, data_set, scaling):
    ...
    def n2Rsum(e, l, s, p, beta):
        ...
        t2 = np.exp(-l14 - l5_0)
        t3fac = np.exp(-l14 - l5_0)
        t3num = np.exp(-l14) + 1 - np.exp(-l14 * (s['S'])) * (s['S'] + 1) ** 2 + \
...
def get_SAD_dynaMETE(data_set, dynaMETE_lambdas, X):
    ...
    sad = sad / Z
    sad = sad / sum(sad)
    #sad = [1/X['S'] for i in n_range] # TODO: for BCI, do the SADs need to have some minimum threshold?
    ...
    if data_set == "BCI":
        ...
        for i in [0, 1, 2, 3]:
            ...
            ax.scatter(np.arange(1, len(empirical_rank_sad[:20]) + 1), empirical_rank_sad[:20], color='red',
                        label='Empirical Rank Abundance', marker='diag', zorder=3)
