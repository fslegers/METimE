...
def get_empirical_RAD(file, census, cluster):
    # Load relevant data
    df = pd.read_csv(file)
...
if __name__ == "__main__":
    ...
    for i in [1, 2, 3, 4, 5, 6, 7, 8, 10
        ...
        # Load data
        og_input = pd.read_csv(f'../../data/BCI_regression_library{ext}_clustered.csv')

        for cluster in [1, 2, 3, 4]:

            input = og_input[og_input['Cluster'] == cluster]
            input.drop(columns=['Cluster'], inplace=True)

            # Compute polynomial coefficients
            alphas, r2_dn, betas, r2_de, scaler = sindy(input)
            functions = get_functions()
            r2s = pd.DataFrame({'r2_dn': [r2_dn], 'r2_de': [r2_de]})
            r2s.to_csv(f'empirical_BCI_r2_tf{ext}_{cluster}.csv', index=False)
            ...
            for census in input['census'].unique():
                ...
                empirical_rad = get_empirical_RAD(f'../../data/BCI_regression_library{ext}_clustered.csv', census, cluster)['abundance']
                ...
                #for w_exp in np.arange(0, 2, 1, dtype=float):
                for w_exp in [1]:
                    for w_base in [1]:
                    #for w_base in np.arange(1, 10, 2):
                        ...
                        #METimE_lambdas = run_optimization(METE_lambdas, macro_var, func_vals, slack_weight=w, maxiter=10)
                        ...
                        if METimE_results['MAE'].values[0] < prev_best_MAE:
                            plot_RADs(empirical_rad, METE_rad, METimE_rad, f'quad_{ext}_census_{census}_cluster_{cluster}', 'Empirical', weight={w}, use_log=True)
                            prev_best_MAE = METimE_results['MAE'].values[0]

            results_df = pd.DataFrame(results_list)
            results_df.to_csv(f'C:/Users/5605407/OneDrive - Universiteit Utrecht/Documents/PhD/Chapter_2/Results/BCI/empirical_BCI_clustered_df/cluster_{cluster}/empirical_BCI_df{ext}.csv', index=False)